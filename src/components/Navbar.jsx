      ...
      >
        ...
        >
          <div className="pt-4 px-6 pb-6 h-full overflow-y-auto">
            <nav className="flex flex-col gap-1">
              {navLinks.map((link, index) => (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `py-4 px-4 text-lg font-medium rounded-xl transition-all duration-300 ${
                      isActive
            ...
            <form onSubmit={handleSearch} className="mt-5">
              <div className="relative">
                ...
                <input
                  ...
                  onChange={(e) => setSearchQuery(e.target.value)}
                  placeholder="搜索产品..."
                  className="w-full pl-11 pr-4 py-3.5 bg-white/70 border border-white/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-forest-500/50 focus:border-transparent transition-all text-sm backdrop-blur"
                />
              </div>
            </form>

            <div className="mt-5 pt-5 border-t border-white/40 flex flex-col gap-2">
              {isLoggedIn && (
                <Link
                  to="/account"
                  className="flex items-center gap-3 py-3.5 px-4 text-base font-medium text-zinc-800 bg-white/50 backdrop-blur rounded-xl hover:bg-white/70 transition-colors"
                >
                  <User size={20} />
                  个人中心
                </Link>
            ...
            <div className="mt-5 pt-5 border-t border-white/40 flex flex-col gap-3 pb-8">
              {isLoggedIn ? (
                <>
                ...
                <>
                  <Link
                    to="/login"
                    className="py-3.5 px-4 text-center font-medium text-zinc-800 bg-white/60 backdrop-blur rounded-xl hover:bg-white/80 transition-colors border border-white/50"
                  >
                    登录
                  </Link>
                  <Link
                    to="/register"
                    className="py-3.5 px-4 text-center font-medium text-white bg-forest-700/90 backdrop-blur rounded-xl hover:bg-forest-800 transition-colors"
                  >
                    注册账号
                  </Link>
                </>
              ...
              <Link
                to="/products"
                className="py-3.5 px-4 text-center font-medium text-forest-900 bg-amber-400/90 backdrop-blur rounded-xl hover:bg-amber-500 transition-colors shadow-soft"
              >
                选购产品
              </Link>
            </div>
